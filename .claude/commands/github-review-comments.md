...
gh api "repos/mhenrixon/locallingo/pulls/<PR_NUMBER>/comments" --paginate
...
' -f owner=mhenrixon -f repo=locallingo -F pr=<PR_NUMBER>
```
...
5. Check the repo's conventions (the README, existing code patterns, and `docs/AGENTS.md` for the docs app) -- project conventions override reviewer preferences. Remember the gem's floor is Ruby 3.2: reject suggestions that need 3.3+/3.4-only syntax.
...
```bash
gh api "repos/mhenrixon/locallingo/pulls/<PR>/comments/<COMMENT_ID>/replies" \
  --method POST \
...
- Reference the repo's documented conventions when applicable
...
- Bot reviewers (CodeRabbit, etc.) sometimes suggest changes that conflict with project conventions -- verify against the repo's actual patterns before accepting
